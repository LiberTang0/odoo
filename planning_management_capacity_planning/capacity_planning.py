...
class capacity_planning(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if addall:
            ...
            emp_ids = emp_obj.search(cr,uid,[('active','=',True),('department_id','=',numdepart)])    
    ...
    def check_update_capacity(self, cr, uid ,ids, context=None):
        ...
        for pl in plan_rec:
            if pl.sel_department.name != None:
                numdepart = pl.sel_department.id
            startdatecapacityplanning = pl.startdate 
            enddatecapacityplanning = pl.enddate
        #fetch employee
        emp_obj = self.pool.get('hr.employee')
        if numdepart == 0:
            emp_ids = emp_obj.search(cr,uid,[('resource_id','!=',None)])
        else:
            emp_ids = emp_obj.search(cr,uid,[('department_id','=',numdepart)])    
        emp_rec = emp_obj.browse(cr,uid,emp_ids) 
        create = False
        ...
        for emp in emp_rec:                  
            ...
            if create:
                ...
                if emp.contract_id.working_hours:
                    id_w=emp.contract_id.working_hours
                #verification si l employe a un contrat faisant partie de la periode du planning demandee 
        ...
        for emp_plan in emp_plan_rec:
            ...
            #if select = 4 start planning = startcontrat
            if datecontratstart:
