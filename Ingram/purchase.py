...
class purchase_order(osv.osv):
    ...
    _columns = {
                'ingramsalesordernumber': fields.char("Numero of order",255,help="Numero of order at Ingram"),
                'ingramsalesorderdate': fields.char("Date the order",255,help="Date the order at Ingram"),
                'sendorder': fields.boolean('Commande envoyee',255),
                'sendmanuel': fields.boolean('Envoyer Manuellement',255),
    ...
    def wkf_confirm_order(self, cr, uid, ids, context=None):
        ...
        if self.pool.get('ingram_config').search(cr,uid,[('xml_active','=','True'),]):
            ...
            if po.partner_id.id == id[0] :
                ...
                for i in ids:
                    if result['warning']['message']=="Command Successful":
                        self.write(cr, uid, [i], {'state' : 'confirmed', 'validator' : uid})
                    elif result['warning']['message']=="Command miss":
    ...
    def test_supplier(self, cr, uid, ids):
        if self.pool.get('ingram_config').search(cr,uid,[('xml_active','=','True'),]):
            idsearch=self.pool.get('ingram_config').search(cr,uid,[('xml_active','=','True'),])
            config=self.pool.get('ingram_config').read(cr,uid,idsearch,['supplier_id'])
            id=config[0]['supplier_id']
    ...
    def envoieCommande(self,cr,uid,ids,context):
        ...
        if response.status == 200:
            ...
            raise osv.except_osv(_('Information!'),_('Connexion failed'))
    ...
    def requeteCode(self,cr,uid,idprod,id):
            ...
            if id_dest:
                ...
                id_dest=self.browse(cr,uid,id).warehouse_id.partner_address_id
                if not id_dest:
    ...
    def handleSlideshowTitle(self,title):
            print  self.getText(title.childNodes)
            return True
