...
class history_command(osv.osv):
    ...
    def miseAjour(self,cr,uid,ids,date,description,statut,skuliste):
        for id in ids:
            ...
            if not idtrouver:
                self.create(cr,uid,{'idmani':id,'description':description,'date':dateExp,'datemaj':datejour})
...
class stock_picking(osv.osv):
    ...
    def handleSlideshowItemDetails(self,cr,uid,ids,noeud):
            ...
            self.pool.get('history.command').miseAjour(cr,uid,ids,date,statusstr,statut,skuliste)
            return True
