                ...
                if objor.state:
                    if objor.state != 'done':
                        if vals.get('name') and conf_rec[0].name.name:
                            self.pool.get(obj.type).write(cr, uid , objor.id,{conf_rec[0].name.name: vals.get('name')}, context=None)                  
                        if vals.get('description') and conf_rec[0].description.name:
                            self.pool.get(obj.type).write(cr, uid , objor.id,{conf_rec[0].description.name: vals.get('description')}, context=None)
                        if vals.get('responsible') and conf_rec[0].responsible.name:
                            self.pool.get(obj.type).write(cr, uid , objor.id,{conf_rec[0].responsible.name: vals.get('responsible')}, context=None) 
                        if vals.get('startingdate') and conf_rec[0].startingdate.name:
                            self.pool.get(obj.type).write(cr, uid , objor.id,{conf_rec[0].startingdate.name: vals.get('startingdate')}, context=None) 
                        if vals.get('endingdate') and conf_rec[0].endingdate.name:
                            self.pool.get(obj.type).write(cr, uid , objor.id,{conf_rec[0].endingdate.name: vals.get('endingdate')}, context=None) 
                        if vals.get('duration') and conf_rec[0].duration.name:
                            self.pool.get(obj.type).write(cr, uid , objor.id,{conf_rec[0].duration.name: vals.get('duration')}, context=None)
                        if vals.get('location') and conf_rec[0].location.name:
                            self.pool.get(obj.type).write(cr, uid , objor.id,{conf_rec[0].location.name: vals.get('location')}, context=None) 
                        if vals.get('attendee') and conf_rec[0].attendee.name:
                            self.pool.get(obj.type).write(cr, uid , objor.id,{conf_rec[0].attendee.name: vals.get('attendee')}, context=None)    
                        if vals.get('mail') and conf_rec[0].mail.name:
                            self.pool.get(obj.type).write(cr, uid , objor.id,{conf_rec[0].mail.name: vals.get('mail')}, context=None)
                else:
                    if vals.get('name') and conf_rec[0].name.name:
                        self.pool.get(obj.type).write(cr, uid , objor.id,{conf_rec[0].name.name: vals.get('name')}, context=None)
                    if vals.get('description') and conf_rec[0].description.name:
...
class shared_calendar_conf(osv.osv):
    ...
    def check_cron(self, cr, uid, context=None):
        ...
        calendar_conf_rec = calendar_conf_obj.browse(cr,uid,calendar_conf_ids)      appel du bouton de configuration
        ...
        if before_month <= 0:
            ...
            date_before = date_now.replace(month=before_month,year=date_now.year-1)
        else:    
            date_before = date_now.replace(month=before_month)
        ...
        if after_month >= 12:
            ...
            date_after = date_now.replace(month=after_month,year=date_now.year+1)
        else:    
            date_after = date_now.replace(month=after_month)
           
        for c in calendar_conf_rec:
            o_obj = self.pool.get(c.object.model)
            o_ids = o_obj.search(cr,uid,[(c.startingdate.name,'>=',date_before.strftime('%Y-%m-%d %H:%M:%S')),(c.startingdate.name,'<=',date_after.strftime('%Y-%m-%d %H:%M:%S'))])
            o_rec = o_obj.browse(cr, uid, o_ids)
            ...
            for o in o_rec:                
                ...
                if id_temp:
                    if c.name.name:
                        self.pool.get('shared.calendar').write(cr, uid , id_temp,{'name': results[c.name.name]}, context=None)                  
                    if c.description.name:
                        self.pool.get('shared.calendar').write(cr, uid , id_temp,{'description': results[c.description.name]}, context=None)
                    if c.responsible.name:
                        self.pool.get('shared.calendar').write(cr, uid , id_temp,{'responsible': results[c.responsible.name][0]}, context=None) 
                    if c.startingdate.name:
                        self.pool.get('shared.calendar').write(cr, uid , id_temp,{'startingdate':results[c.startingdate.name]}, context=None)  
                    if c.endingdate.name:
                        self.pool.get('shared.calendar').write(cr, uid , id_temp,{'endingdate':results[c.endingdate.name]}, context=None) 
                    if c.duration.name:
                        self.pool.get('shared.calendar').write(cr, uid , id_temp,{'duration':results[c.duration.name]}, context=None)
                    if c.location.name:
                        self.pool.get('shared.calendar').write(cr, uid , id_temp,{'location':results[c.location.name]}, context=None) 
                    if c.attendee.name:
                        self.pool.get('shared.calendar').write(cr, uid , id_temp,{'attendee':results[c.attendee.name]}, context=None)   
                    if c.mail.name:
                        self.pool.get('shared.calendar').write(cr, uid , id_temp,{'mail':results[c.mail.name]}, context=None) 
                else:
                    i=self.pool.get('shared.calendar').create(cr, uid ,{'name': results[c.name.name],'startingdate':results[c.startingdate.name],'object_id':id,'type':c.object.model}, context=None)
                    '''if c.description.name:
...
class shared_calendar_conf_information(osv.osv):
    ...
    _columns = {
        'conf_id': fields.many2one('shared.calendar.conf', 'Configuration', readonly=True, invisible=True),
    ...
    def on_change_object(self, cr, uid, ids, object_id, context=None):
        ...
        #duration
        if obj in ("crm.meeting","crm.phonecall"):
